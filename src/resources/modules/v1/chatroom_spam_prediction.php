<?php
    ...
    class chatroom_spam_prediction extends Module implements Response
    {
        ...
        private function processQuota(): bool
        {
            ...
            {
                $this->access_record->setVariable("SPAM_CHECKS", 0);
            }

            // If the user has unlimited, ignore the check.
            if((int)$this->access_record->Variables["MAX_SPAM_CHECKS"] > 0)
            {
                // If the current sessions are equal or greater
                if($this->access_record->Variables["SPAM_CHECKS"] >= $this->access_record->Variables["MAX_SPAM_CHECKS"])
                {
